...
async function getNewSkySchedules() {
    ...
    const url =
        `https://newsky.app/api/airline/${NEWSKY_AIRLINE_ID}/schedules`;
    ...
    if (!response.ok) {
        ...
        console.error(
            "NewSky schedules API error:",
            response.status,
            text
        );

        throw new Error(
            `NewSky schedules API returned HTTP ${response.status}`
        );
    }
    ...
    try {
        ...
        console.error(
            "NewSky schedules returned invalid JSON:"
        );
        ...
        throw new Error(
            "NewSky schedules endpoint returned invalid JSON."
        );
    }
...
// ============================================================
// EXTRACT NEWSKY SCHEDULES
// ============================================================

function extractNewSkySchedules(data) {
    ...
    if (
        ...
        /*
         * NewSky returns schedules as an object
    ...
    // ============================================================
// NORMALIZE NEWSKY SCHEDULE
// ============================================================

function normalizeNewSkySchedule(
    schedule
) {
    ...
    if (
        ...
        schedule.airframes.length > 0
    ) {

        aircraft =
            schedule.airframes.join(
                ", "
            );
...
// ============================================================
// SCHEDULE AIRPORT CODE
// ============================================================

function getScheduleAirportCode(
    value,
    depth = 0
) {
...
app.get(
    ...
    async (req, res) => {
        ...
        try {
            ...
            const schedules =
                extractNewSkySchedules(
                    data
                );


            console.log(
                `NewSky returned ${schedules.length} schedules.`
            );


            // ------------------------------------------------
            // CONVERT SCHEDULES TO ROUTES
            // ------------------------------------------------

            const routes =
                schedules
                    .map(
                        normalizeNewSkySchedule
                    )
                    .filter(
                        route =>
                            route.departure &&
            ...
            finalRoutes.sort(
                (
                    ...
                    const aKey =
                        `${a.departure}-${a.arrival}-${a.flightNumber || ""}`;

                    const bKey =
                        `${b.departure}-${b.arrival}-${b.flightNumber || ""}`;
...
// ============================================================
// PostgreSQL route endpoint intentionally removed. Routes use NewSky schedules above.
